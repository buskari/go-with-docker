...
func TestRouter(t *testing.T) {
	// Instantiate the router using the constructor function defined previously
	r := newRouter()
	...
	// In the next few lines, the response body is read and converted to string
	...
	// If it does happen to be "Hello, World!", then it confirms that the route is correct
	if respString != expected {
		t.Errorf("Response shoud be %s, got %s", expected, respString)
	}
}
